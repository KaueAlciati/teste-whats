window.FincontrolLayout = (() => {
  ...
  function buildSidebar(user, activePage = "dashboard") {
    ...
    const renderLink = ([key, href, lucide, label]) => `<a class="nav-link sidebar-link ${activePage === key ? "active is-active" : ""}" href="${href}">${icon(lucide)}<span>${label}</span></a>`;
    return `<div class="sidebar-inner">
      <a class="navbar-brand sidebar-brand reference-brand" href="/dashboard"><span class="reference-brand-mark">FC</span><span><strong>Fincontrol</strong><small>IA Financeira</small></span></a>
      <hr class="sidebar-divider">
      <nav class="navbar-nav reference-nav" aria-label="Menu principal">
      ...
      <div class="sidebar-footer mt-auto">
        <a class="nav-link sidebar-link ${activePage === "settings" ? "active is-active" : ""}" href="/configuracoes">${icon("settings")}<span>Configurações</span></a>
        <button id="sidebar-logout" class="btn btn-ghost sidebar-logout" type="button">${icon("log-out")}<span>Sair</span></button>
  ...
  function buildTopbar(user, period) {
    return `<div class="container-fluid reference-topbar-inner">
      <button id="sidebar-toggle" class="mobile-menu-button btn btn-link" type="button" aria-label="Abrir menu" aria-expanded="false">${icon("menu")}</button>
      <label class="topbar-search"><i data-lucide="search" aria-hidden="true"></i><input type="search" placeholder="Buscar no painel" autocomplete="off" aria-label="Buscar no painel"></label>
      <div class="topbar-actions ms-auto">
        <label class="period-select"><span class="topbar-period-label">Período</span><select id="period-select" aria-label="Selecionar período">
          <option value="current_month" ${period === "current_month" ? "selected" : ""}>Este mês</option><option value="previous_month" ${period === "previous_month" ? "selected" : ""}>Mês passado</option><option value="current_week" ${period === "current_week" ? "selected" : ""}>Esta semana</option><option value="previous_week" ${period === "previous_week" ? "selected" : ""}>Semana passada</option><option value="current_year" ${period === "current_year" ? "selected" : ""}>Este ano</option><option value="today" ${period === "today" ? "selected" : ""}>Hoje</option>
        </select></label>
  ...
  async function renderShell(user, period, onPeriodChange, onLogout) {
    ...
    const activePage = window.location.pathname.includes("configuracoes") ? "settings" : "dashboard";
    sidebar?.classList.add("reference-sidebar");
    topbar?.classList.add("reference-topbar");
    if (sidebar) sidebar.innerHTML = buildSidebar(user, activePage);
    if (topbar) topbar.innerHTML = buildTopbar(user, period);
    const toggle = document.getElementById("sidebar-toggle");
    const backdrop = document.getElementById("sidebar-backdrop");
    const closeMenu = () => { document.body.classList.remove("sidebar-open"); if (backdrop) backdrop.hidden = true; toggle?.setAttribute("aria-expanded", "false"); };
